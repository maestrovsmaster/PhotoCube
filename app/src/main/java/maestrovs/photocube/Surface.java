...
public class Surface implements Telo{
   ...
   // private int[] textureIDs = new int[numFaces];
    ...
    public Surface(Context context, Coord3d coord3d, float widhtX, float lenthZ, int koef, int textureResId, int h_mapResId) {
    ...
    private void calculateSurface()
    {
        ...
        GetPixelColor getPixelColor = new GetPixelColor(context,  h_mapResId);
       ArrayList<ArrayList<MyPixel>> pixelsList =  getPixelColor.getPixelsList();




        float numFfl = numF;
        float prSurf=numFfl/100;

        float numPixelSize=pixelsList.size();
        float prPix = numPixelSize/100;
        ...
        for(int iz=0;iz<numF+1;iz++)
        {
            ...
            for(int ix=0;ix<numF+1;ix++){
                ...
                float xf=ix;
                float currentPrX=xf/prSurf;

                float yf=iz;
                float currentPrY=100-yf/prSurf;

                int currentPixelX = (int) (currentPrX*prPix);
                int currentPixelY = (int) (currentPrY*prPix);

                Log.d("my","current pixel coord: x="+currentPixelX+" y="+currentPixelY);

                int r=0,g=0,b=0,a=0;

                if(currentPixelY<pixelsList.size()&&currentPixelX<pixelsList.size()){


                MyPixel pixel = pixelsList.get(currentPixelY).get(currentPixelX);
